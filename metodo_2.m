...
%     soggetto = soggetto(1:end-4);
    ...
    start = start*25;
    finish = finish*25;
    metodo = 'otsu_skin_pca'; %in alternativa usare kmeans o arbib o jrmogskin o skin_detector3
    %green = zeros(j,60);
    skin = skin_otsu2(soggetto,start,finish);
    %skin = skin_detector(soggetto,start,finish,metodo);
    red = average_green(skin(:,:,1,:));
    green = average_green(skin(:,:,2,:));
    blue = average_green(skin(:,:,3,:));
    %% Interpolazione red
    red(isnan(red))=0;
    [col,index,value]=find(red);
    ...
    for i=1:5:max(index)-1
        ...
        red(z)=interp1(ind,val,z);
        j=j+1
    end
    %% Interpolazione green
     green(isnan(green))=0;
    ...
    bpFilt = designfilt('bandpassfir','FilterOrder',32, ...
         'CutoffFrequency1',0.75,'CutoffFrequency2',3, ...
